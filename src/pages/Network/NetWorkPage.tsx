...
export default function NetWorkPage() {
  ...
  const ipconfig = configData.data?.ipconfig || [];
  const publicIp = configData.data?.publicIp || '';
  ...
  const handleChangeIp = async () => {
    ...
    if (!regex.test(changeIp)) {
      toast({
        variant: 'destructive',
        title: 'IP 주소를 정상적으로 입력해주세요.',
        description: 'IP 주소는 192.168.10-12.X 형식이어야 합니다. X는 0에서 255 사이의 숫자여야 합니다.',
      });
      return;
    }

    const response = await window.electron.changeIp({ interfaceName: currentNetworkConfig.interfaceName, changeIp });
    if (!response.success) {
        ...
        <TabsContent value="IpChange">
          <Card className="w-full">
            <CardHeader>
              ...
              <CardDescription>192.168.12.XXX 대역만 변경 ( Hyper-V ) </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid w-full items-center gap-4">
                <div className="flex flex-col space-y-1.5">
                  <Label>현재 IP</Label>
                  <Input id="currentIp" disabled defaultValue={currentNetworkConfig?.ipv4} />
                </div>
                <div className="flex flex-col space-y-1.5">
                  <Label>변경할 IP</Label>
